...
- **Two of the four enrichers are off in the public demo, and binding a key does not turn them on.** VirusTotal and AbuseIPDB need an API key, so the console reports them `disabled` and redistributes their weight rather than filling them in with a plausible number. Binding `VIRUSTOTAL_API_KEY` and `ABUSEIPDB_API_KEY` is not enough to reach them: `/triage` is public, and a public route that spends a paid key on any IP a stranger posts is a free proxy for a metered service. The key would never leak and the bill would still arrive. So the keyed sources also require an `x-enrich-token` header matching the `ENRICH_TOKEN` secret, which an operator has and a visitor does not. Unset, it fails closed. `src/index.test.ts` pins that.
